...
def publish_to_senso(person_name: str, promises_with_verdicts: list) -> str:
    ...
    try:
        ...
        prompt_result = subprocess.run([
            ...
            "--output", "json", "--quiet"
        ], capture_output=True, text=True, env=senso_env)

        # Extract JSON part (stdout has extra text before JSON)
        stdout = prompt_result.stdout
        json_start = stdout.find("{")
        prompt_id = json.loads(stdout[json_start:]).get("prompt_id", "") if json_start >= 0 else ""
        if not prompt_id:
            print(f"  ⚠️ No prompt ID returned")
            return ""

        # Step 2: Write content to temp file and ingest
        with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False, prefix=f"receipts-{person_name.replace(' ','-')}-") as f:
            f.write(content)
            temp_path = f.name
        ...
        if ingest_result.returncode == 0:
            print(f"  → Content ingested into knowledge base")
        else:
            print(f"  ⚠️ Ingest warning: {ingest_result.stderr[:100]}")
        ...
        pub_result = subprocess.run([
            ...
            "--data", json.dumps({
                ...
                "summary": f"AI-powered promise tracking for {person_name}. Did they keep their word?"
            }),
            "--output", "json", "--quiet"
        ], capture_output=True, text=True, env=senso_env)

        if pub_result.returncode == 0:
            page_url = f"https://geo.senso.ai/published/{person_name.lower().replace(' ','-')}"
            print(f"  → Published: {page_url}")
            return page_url
        else:
            print(f"  ⚠️ Publish error: {pub_result.stderr[:300]}")
            # Still return a URL — content is in the KB
            return f"https://geo.senso.ai/published"

    except Exception as e:
        print(f"  ⚠️ Senso publish failed: {e}")
        return ""
...
def run_receipts(person_name: str):
    ...
    # Publish all verdicts to Senso at once
    page_url = publish_to_senso(person_name, promises_with_verdicts)
